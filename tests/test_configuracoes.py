...
def test_listar_configuracoes_traz_as_chaves_semeadas(client, auth_headers):
    ...
    assert len(resposta.json()) == 36  # 13 v0.3.4 + PRAZO_EXPERIENCIA_DIAS (v1.2) + PRAZO_RETENCAO_DESLIGADO_DIAS (v1.4) + PRAZO_RECADASTRAMENTO_DIAS (v1.8) + DATA_MAGNA/VERSICULOS_BASE/ORACAO_OFICIAL (v2.0) + CHAVE_PIX/NOME_BENEFICIARIO_PIX/CIDADE_BENEFICIARIO_PIX (v3.2) + DIAS_LEMBRETE_MENSALIDADE (v3.2.1 adaptado) + DIAS_ATRASO_LEMBRETE (v3.2.2) + VALOR_MINIMO_EXIGE_COTACAO (v3.3) + HORIZONTE_FLUXO_CAIXA_MESES (v3.5) + HORA_INICIO_EXPEDIENTE/HORA_FIM_EXPEDIENTE/PERCENTUAL_ALERTA_FRACIONAMENTO/VALOR_ALERTA_FORNECEDOR_NOVO/QUANTIDADE_ALERTA_ESTORNOS_MESMO_USUARIO/DIAS_ALERTA_TROCA_DADOS_BANCARIOS (v3.7) + TEXTO_CONSENTIMENTO_LGPD_INSCRICAO/VERSAO_TEXTO_CONSENTIMENTO_LGPD_INSCRICAO/URL_BASE_SITE_PUBLICO (v4.6) + PRAZO_CONFIRMACAO_LISTA_ESPERA_HORAS (v4.7)
