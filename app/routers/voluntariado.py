...
def registrar_horas_voluntariado(
    ...
    registro = RegistroHorasVoluntariado(
        id_termo=termo.id_termo, data=datetime.combine(dados.data, datetime.min.time()), horas=dados.horas,
        descricao_atividade=dados.descricao_atividade, id_projeto=dados.id_projeto,
        id_usuario_registrou=usuario.id_usuario,
    )
    db.add(registro)
    db.commit()
    ...
    return {"mensagem": "Horas registradas.", "id_registro": registro.id_registro}
